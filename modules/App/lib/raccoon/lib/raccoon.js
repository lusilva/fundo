...
class Raccoon {
    ...
    liked(userId, itemId, callback) {
        ...
        that.client.sismember(
            [that.config.city, itemId, 'liked'].join(":"),
            userId,
            function (err, results) {
                ...
                if (results === 0) {
                    that.client.zincrby(
                        [that.config.city, 'mostLiked'].join(":"),
                        1,
                        itemId
                    );
                }
                that.client.sadd(
                    [that.config.city, userId, 'liked'].join(':'),
                    itemId,
                    function (err) {
                        that.client.sadd(
                            [that.config.city, itemId, 'liked'].join(':'),
    ...
    disliked(userId, itemId, callback) {
        let that = this;
        that.client.sismember(
            [that.config.city, itemId, 'disliked'].join(":"),
            userId,
            function (err, results) {
                if (results === 0) {
                    that.client.zincrby([that.config.city, 'mostDisliked'].join(":"), 1, itemId);
                }
                that.client.sadd(
                    [that.config.city, userId, 'disliked'].join(':'),
                    itemId,
                    function (err) {
                        that.client.sadd(
                            [that.config.city, itemId, 'disliked'].join(':'),
    ...
    _jaccardCoefficient(userId1, userId2, callback) {
        ...
        that.client.sinter([that.config.city, userId1, 'liked'].join(":"), [that.config.city, userId2, 'liked'].join(":"), function (err, results1) {
            // retrieving a set of the users dislike incommon
            that.client.sinter([that.config.city, userId1, 'disliked'].join(":"), [that.config.city, userId2, 'disliked'].join(":"), function (err, results2) {
                // retrieving a set of the users like and dislikes that they disagree on
                that.client.sinter([that.config.city, userId1, 'liked'].join(":"), [that.config.city, userId2, 'disliked'].join(":"), function (err, results3) {
                    // retrieving a set of the users like and dislikes that they disagree on
                    that.client.sinter([that.config.city, userId1, 'disliked'].join(":"), [that.config.city, userId2, 'liked'].join(":"), function (err, results4) {
    ...
    _updateSimilarityFor(userId, cb) {
        ...
        similaritySet = [that.config.city, userId, 'similaritySet'].join(":");
        // creating a combined set with the all of a users likes and dislikes
        that.client.sunion([that.config.city, userId, 'liked'].join(":"), [that.config.city, userId, 'disliked'].join(":"), function (err, userRatedItemIds) {
            ...
            if (userRatedItemIds.length > 0) {
                ...
                itemLikeDislikeKeys = _.map(userRatedItemIds, function (itemId, key) {
                    ...
                    itemLiked = [that.config.city, itemId, 'liked'].join(":");
                    // key for the item being disliked
                    itemDisliked = [that.config.city, itemId, 'disliked'].join(":");
    ...
    _predictFor(userId, itemId, callback) {
        ...
        var prediction = 0.0;
        var similaritySet = [that.config.city, userId, 'similaritySet'].join(':');
        var likedBySet = [that.config.city, itemId, 'liked'].join(':');
        var dislikedBySet = [that.config.city, itemId, 'disliked'].join(':');
    ...
    _updateRecommendationsFor(userId, cb) {
        ...
        var tempSet = [that.config.city, userId, 'tempSet'].join(":");
        var tempDiffSet = [that.config.city, userId, 'tempDiffSet'].join(":");
        var similaritySet = [that.config.city, userId, 'similaritySet'].join(":");
        var recommendedSet = [that.config.city, userId, 'recommendedSet'].join(":");
        ...
        that.client.zrevrange(similaritySet, 0, that.config.nearestNeighbors - 1, function (err, mostSimilarUserIds) {
            ...
            that.client.zrange(similaritySet, 0, that.config.nearestNeighbors - 1, function (err, leastSimilarUserIds) {
                ...
                _.each(mostSimilarUserIds, function (id, key) {
                    setsToUnion.push([that.config.city, id, 'liked'].join(":"));
                });
                ...
                if (that.config.factorLeastSimilarLeastLiked) {
                    _.each(leastSimilarUserIds, function (id, key) {
                        setsToUnion.push([that.config.city, id, 'disliked'].join(":"));
                    });
                }
                ...
                if (setsToUnion.length > 0) {
                    ...
                    async.each(setsToUnion,
                        ...
                        function (err) {
                            ...
                            that.client.sdiff(tempSet, [that.config.city, userId, 'liked'].join(":"), [that.config.city, userId, 'disliked'].join(":"), function (err, notYetRatedItems) {
    ...
    _updateWilsonScore(itemId, callback) {
        ...
        var scoreBoard = [that.config.city, 'scoreBoard'].join(":");
        var likedBySet = [that.config.city, itemId, 'liked'].join(':');
        var dislikedBySet = [that.config.city, itemId, 'disliked'].join(':');
    ...
    bestRated(callback) {
        this.client.zrevrange([this.config.city, 'scoreBoard'].join(":"), 0, -1, function (err, results) {
            callback(results);
        });
    };

    worstRated(callback) {
        this.client.zrange([this.config.city, 'scoreBoard'].join(":"), 0, -1, function (err, results) {
            callback(results);
        });
    };

    bestRatedWithScores(numOfRatings, callback) {
        this.client.zrevrange([this.config.city, 'scoreBoard'].join(":"), 0, numOfRatings, 'withscores', function (err, results) {
            callback(results);
        });
    };

    mostLiked(callback) {
        this.client.zrevrange([this.config.city, 'mostLiked'].join(":"), 0, -1, function (err, results) {
            callback(results);
        });
    };

    mostDisliked(callback) {
        this.client.zrevrange([this.config.city, 'mostDisliked'].join(":"), 0, -1, function (err, results) {
            callback(results);
        });
    };

    mostSimilarUsers(userId, callback) {
        this.client.zrevrange([this.config.city, userId, 'similaritySet'].join(":"), 0, -1, function (err, results) {
            callback(results);
        });
    };

    leastSimilarUsers(userId, callback) {
        this.client.zrange([this.config.city, userId, 'similaritySet'].join(":"), 0, -1, function (err, results) {
            callback(results);
        });
    };

    likedBy(itemId, callback) {
        this.client.smembers([this.config.city, itemId, 'liked'].join(':'), function (err, results) {
            callback(results);
        });
    };

    likedCount(itemId, callback) {
        this.client.scard([this.config.city, itemId, 'liked'].join(':'), function (err, results) {
            callback(results);
        });
    };

    dislikedBy(itemId, callback) {
        this.client.smembers([this.config.city, itemId, 'disliked'].join(':'), function (err, results) {
            callback(results);
        });
    };

    dislikedCount(itemId, callback) {
        this.client.scard([this.config.city, itemId, 'disliked'].join(':'), function (err, results) {
            callback(results);
        });
    };

    allLikedFor(userId, callback) {
        this.client.smembers([this.config.city, userId, 'liked'].join(":"), function (err, results) {
            callback(results);
        });
    };

    allDislikedFor(userId, callback) {
        this.client.smembers([this.config.city, userId, 'disliked'].join(":"), function (err, results) {
            callback(results);
        });
    };

    allWatchedFor(userId, callback) {
        this.client.sunion([this.config.city, userId, 'liked'].join(":"), [this.config.city, userId, 'disliked'].join(":"), function (err, results) {
            callback(results);
        });
    }
}
